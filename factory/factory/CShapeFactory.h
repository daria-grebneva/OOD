...
private:
	...
	std::unique_ptr<CRegularPolygon> CreateRegularPolygon(std::istream& args);
	std::unique_ptr<CTriangle> CreateTriangle(std::istream& args);
	std::unique_ptr<CEllipse> CreateEllipse(std::istream& args);

	const ActionMap m_action = {
		{ CREATE_CIRCLE, bind(&CShapeFactory::CreateCircle, this, std::placeholders::_1) },
